...
export default function ConceptMapper({ data }: ConceptMapperProps) {
  ...
  const handleNodeClick = useCallback(
    (node: any) => {
      ...
      if (action === '1') {
        ...
        setNodes((nds: any[]) => nds.filter((n: { id: any; }) => n.id !== node.id));
        setEdges((eds: any[]) => eds.filter((e: { source: any; target: any; }) => e.source !== node.id && e.target !== node.id));
      }
    },
  ...
  const deleteSelectedElements = useCallback(() => {
    setNodes((nds: any[]) => nds.filter((node: { selected: any; }) => !node.selected));
    setEdges((eds: any[]) => eds.filter((edge: { selected: any; }) => !edge.selected));
  }, [setNodes, setEdges]);
      ...
      <ReactFlowProvider>
        <div className="w-full h-full" ref={reactFlowWrapper}>
          <ReactFlow
            ...
            onNodeClick={(_: any, node: any) => handleNodeClick(node)}
          >
